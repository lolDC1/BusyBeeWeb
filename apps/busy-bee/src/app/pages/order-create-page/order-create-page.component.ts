...
export class OrderCreatePageComponent implements OnInit {
  ...
  constructor(
    ...
    this.form = new FormGroup({
      ...
      time: new FormControl(<TaskTimes>TaskTimes.Anytime, {
        nonNullable: true,
        validators: [Validators.required],
      }),
      ...
      .subscribe({
        next: (model) => {
          ...
          this.orderAddressDataTemplate!.dataTemplateItems.forEach((item) => {
            this.form.controls.orderAddressItems.addControl(
              item.id,
          ...
          this.paymentDataTemplate!.dataTemplateItems.forEach((item) => {
            if (item.type == DataTemplateType.MultipleSelection) {
              item.selection?.forEach((itemValue) => {
                this.form.controls.paymentItems.addControl(
                  itemValue.id,
              ...
              if (
                item.type == DataTemplateType.SingleSelection &&
                item.selection?.some(() => true)
              ) {
                this.form.controls.paymentItems
                  .get(item.id)
                  ?.setValue(item.selection[0].id);
              }
            }
          });
        ...
        error: (_) =>
          console.log(
            'ERROR. Not found data template for categoryId: ' + this.categoryId
          ),
      });
  ...
  calculateCost() {
    this.form.controls.cost.setValue(
      (this.orderAddressDataTemplate?.estimatedCost ?? 0) +
  ...
  getSelectionsByFormControlName(name: string): string[] {
    ...
    // multiple selection
    return this.paymentDataTemplate!.dataTemplateItems.concat(
      this.orderAddressDataTemplate!.dataTemplateItems
    )
      .find((x) => x.id == name)!
      .selection!.map((x) => x.id)
      .filter(
        (id) =>
          obj.paymentItems[id] === true || obj.orderAddressItems[id] === true
      );
  }
  ...
  publish() {
    ...
    const dataTemplateItems = this.paymentDataTemplate.dataTemplateItems.concat(
      this.orderAddressDataTemplate.dataTemplateItems
    );

    this.taskService
      .create(<TaskCreateCommandModel>{
        ...
        selections: dataTemplateItems
          .filter((x) => x.type != DataTemplateType.String)
          .map(
            (x) =>
              <TaskSelectionDataValueResponse>{
                dataTemplateItemId: x.id,
                values: this.getSelectionsByFormControlName(x.id),
              }
          ),
        strings: dataTemplateItems
          .filter((x) => x.type == DataTemplateType.String)
          .map(
            (x) =>
              <TaskStringDataValueResponse>{
                dataTemplateItemId: x.id,
      ...
      .pipe(untilDestroyed(this))
      .subscribe((model) => {
        this.router.navigate(['tasks', model.id]);
      });
  }
}
